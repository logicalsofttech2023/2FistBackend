...
const Transaction=require("../models/transjection_models");
...
const dashboardData_api = async (req, res) => {
	try {
	  ...
	  const verifiedCoordinatorCount = await Coordinator.countDocuments({Status: 1 });
...
//transaction list
const userTransaction_list=async(req,res)=>{
	try{
		const data = await Transaction.find({userId:{$exists:true}}).populate('userId').sort({createdAt:-1});
			
		if(!data || data.length===0){
			return res.status(404).json({result:false,message:"Record not found"})
		}
		res.status(200).json({result:true,message:"User transaction list got successfully",data:data})
...
const coordinatorTransactionList=async(req,res)=>{
	try{
		const data = await Transaction.find({coordinatorId:{$exists:true}}).populate('coordinatorId').sort({createdAt:-1});
			
		if(!data || data.length===0){
			return res.status(404).json({result:false,message:"Record not found"})
		}
		res.status(200).json({result:true,message:"Coordinator transaction list got successfully",data:data})
...
const userWithdraw_list=async(req,res)=>{
	try{
		const data = await Withdraw.find({userId:{$exists:true}}).populate('userId').sort({createdAt:-1});
			
		if(!data || data.length===0){
			return res.status(404).json({result:false,message:"Record not found"})
		}
		res.status(200).json({result:true,message:"User Withdraw list got successfully",data:data})
...
const coordinatorWithdrawList=async(req,res)=>{
	try{
		const data = await Withdraw.find({coordinatorId:{$exists:true}}).populate('coordinatorId').sort({createdAt:-1});
			
		if(!data || data.length===0){
			return res.status(404).json({result:false,message:"Record not found"})
		}
		res.status(200).json({result:true,message:"Coordinator Withdraw list got successfully",data:data})
...
//approve userWithdrawr request
const approveUserWithdrawRequest=async(req,res)=>{
	try{
		const {userId,requestId,status}=req.body;
		if(!userId || !requestId){
			return res.status(400).json({result:false,message:"userId,requestId,status is required"})
		}
		const users=await User.findById({_id:userId});
		const data=await Withdraw.findById({_id:requestId});
		if(status==="Rejected"){
			//change status
		await Withdraw.findByIdAndUpdate({_id:requestId},{status},{new:true});
		return res.status(200).json({result:true,message:"Rejected successfully"})

		}
		//insert data  in tarnsaction collection
		let transjectionId=generateTransjectionNumber();
		const insert_data=new Transaction({userId,transjectionId,amount:data.amount,tranjectionType:"Withdraw"});
		await insert_data.save();
		let remaingAmount=Number(users.userWallet - data.amount);
		
		//update date in user collection
		await User.findByIdAndUpdate({_id:userId},{userWallet:remaingAmount},{new:true});
		//change status
		await Withdraw.findByIdAndUpdate({_id:requestId},{status:"Approved"},{new:true});
		res.status(200).json({result:true,message:"Approved successfully"})
...
const approveCoordinatorWithdrawRequest=async(req,res)=>{
	try{
		const {coordinatorId,requestId,status}=req.body;
		if(!coordinatorId || !requestId){
			return res.status(400).json({result:false,message:"coordinatorId,requestId,status is required"})
		}

		const coordinator=await Coordinator.findById({_id:coordinatorId});
		const data=await Withdraw.findById({_id:requestId});
		if(status==="Rejected"){
			//change status
		await Withdraw.findByIdAndUpdate({_id:requestId},{status},{new:true});
		return res.status(200).json({result:true,message:"Approved successfully"})

		}
		
		//insert data  in tarnsaction collection
		let transjectionId=generateTransjectionNumber();
		const insert_data=new Transaction({coordinatorId,transjectionId,amount:data.amount,tranjectionType:"Withdraw"});
		await insert_data.save();
		let remaingAmount=Number(coordinator.coordinatorWallet - data.amount);
		
		//update date in user collection
		await Coordinator.findByIdAndUpdate({_id:coordinatorId},{coordinatorWallet:remaingAmount},{new:true});
		//change status
		await Withdraw.findByIdAndUpdate({_id:requestId},{status:"Approved"},{new:true});
		res.status(200).json({result:true,message:"Approved successfully"})
...
const depositeCount = async (req, res) => {
	try {
		// Fetching transactions with transjectionType "Wallet"
		const transactions = await Transaction.find({ tranjectionType: "Wallet" });

		// Calculating total deposit amount
		const total_deposite = transactions.reduce((acc, transaction) => acc + transaction.amount, 0);

		// Counting pending deposits
		const pending_deposite = await Transaction.countDocuments({ transjectionType: "Wallet", status: "Pending" });

		// Counting rejected deposits
		const rejected_deposite = await Transaction.countDocuments({ transjectionType: "Wallet", status: "Rejected" });
...
const withdrawCount=async(req,res)=>{
	try {
		// Fetching transactions with transjectionType "Wallet"
		const transactions = await Withdraw.find({status: "Approved" });

		// Calculating total deposit amount
		const total_withdraw = transactions.reduce((acc, transaction) => acc + transaction.amount, 0);

		// Counting pending deposits
		const pending = await Withdraw.find({status: "Pending"});
		const total_pending = pending.reduce((acc, transaction) => acc + transaction.amount, 0);

		// Counting rejected deposits
		const rejected_withdraw = await Withdraw.find({status: "Rejected" });
		const total_rejected=rejected_withdraw.reduce((sum,item)=>sum + item.amount,0);
     const data={
		total_withdraw,
		total_pending,
		total_rejected,
...
const  userBetsList=async(req,res)=>{
  ...
  const coordinatorMatcheLists=async(req,res)=>{
	try{
		const {coordinatorId}=req.body;
		if(!coordinatorId){
return res.status(400).json({result:false,message:"coordinatorId is required"})
		}
		const matchlist=await Match.find({coordinatorId});
		if(!matchlist || matchlist.length==0){
			res.status(400).json({result:"false",message:"Record not found"})
		}

		
		res.status(200).json({result:"true",message:"Match list got sucessfully",data:matchlist})
...
const matcheBetsList=async(req,res)=>{
	try{
		const {matchId}=req.body;
		if(!matchId){
return res.status(400).json({result:false,message:"matchId is required"})
		}
		const matchlist=await Bets.find({matchId}).populate('matchId coordinatorId userId');
		if(!matchlist || matchlist.length==0){
			res.status(400).json({result:"false",message:"Record not found"})
		}
   ...
   const coordinatorCommissnionList=async(req,res)=>{
	try{
		
		const commission=await Comission.find({}).sort({createdAt:-1}).populate('matchId coordinatorId');
		if(!commission || commission.length==0){
			res.status(400).json({result:"false",message:"Record not found"})
		}

		res.status(200).json({result:"true",message:"Commission list got sucessfully",data:commission})
...
module.exports = {
   ...
   allBetUsersList,
   dashboardData_api,
   userTransaction_list,
   coordinatorTransactionList,
   userWithdraw_list,
   coordinatorWithdrawList,
   approveUserWithdrawRequest,
   approveCoordinatorWithdrawRequest,   
    depositeCount,
   withdrawCount,	
    userBetsList,	
    coordinatorMatcheLists,
    matcheBetsList,
   coordinatorCommissnionList,
