...
const MatchList=async(req,res)=>{
  try{
    ...
    const Match_list = await Match.find({
      categoryId,
      $or: [{ matchStatus: "Live" }, { matchStatus: "Upcoming" }], // Correct $or syntax
...
const coordinatorDashboardCount = async (req, res) => {
  try {
    ...
    const coordinatorObjectId = new mongoose.Types.ObjectId(coordinatorId);
    ...
    const [
      ...
      Match.countDocuments({ coordinatorId:coordinatorObjectId, matchStatus: "Live" }),
      Match.countDocuments({coordinatorId:coordinatorObjectId,matchStatus:"Completed"}),
      Match.countDocuments({ coordinatorId:coordinatorObjectId, matchStatus: "Abandon" }),
      Match.countDocuments({ coordinatorId:coordinatorObjectId, start_date: formattedDate }),
      Match.countDocuments({ coordinatorId:coordinatorObjectId, matchStatus: "Upcoming" })
    ]);
...
const coordinatorComissionList = async (req, res) => {
  try {
    ...
    const data= await Comission.find({coordinatorId}).populate('matchId').sort({_id:-1});
...
await transjectionData.save();
  ...
  await User.findByIdAndUpdate({_id:userId},{userWallet:remaining_amount},{new:true});
...
const updateMatcheStatus=async(req,res)=>{
    ...
    if(matchStatus=="Live"){
      await Bets.updateMany({matchId},{match_status:matchStatus},{new:true});
    }
...
const updateMatcheResult = async (req, res) => {
  try {
    ...
    // Update bet amount
    await Bets.updateMany({matchId,matched:true}, {
      $set: {winnerTeam,match_status:"Completed"},
    });
     
    // refund money of all bets user
    const data=await Bets.find({matchId,matched:true});
   // Create a map to sum up refund amounts per user
const refundMap = {};

data.forEach((bet) => {
  if (!refundMap[bet.userId]) {
    refundMap[bet.userId] = 0;
  }
  refundMap[bet.userId] += bet.amount; // Sum amounts per user
});


// Convert refund data into bulk update operations
const bulkOps = Object.entries(refundMap).map(([userId, amount]) => ({
  updateOne: {
    filter: { _id: userId }, // Match user by _id
    update: { $inc: { userWallet: amount } } // Add amount to userWallet
  }
}));

// Execute bulk update if there are users to update
if (bulkOps.length > 0) {
  await User.bulkWrite(bulkOps);
  console.log("User wallets updated successfully!");
} else {
  console.log("No users found for refund.");
}


   return res.status(200).json({
      result: "true",
      message: "Match status updated and amount refunded successfully.",
    ...
    for (const bet of bets) {
      ...
      // Update bet amount
      await Bets.updateMany({matchId,outcome:winnerTeam}, {
        $set: {winning_amount: payout,winnerTeam,match_status:"Completed"},
      });
...
const findingMatched_bet = async (req, res) => {
  try {
    ...
    if (!matchId || !amount || !userId || !outcome) {
    ...
    const bet_list = await Bets.findOne({
      ...
      outcome: { $ne: outcome },
      matched: false,
      userId: { $ne: userId } // Exclude bets from the same user
    }).populate('userId');
    ...
    if (bet_list) {
      ...
      // transaction insert data in database
      const transId = generateTransjectionNumber();
      const transjectionData = new Transjection({ userId, amount, tranjectionType: "Apply bet", transjectionId: transId, matchId });
      await transjectionData.save();
...
const MatchedBet_userDetails=async(req,res)=>{
  try{
   ...
   //agora live streaming
   const liveToken=await Live.findOne({matchId});
   if(liveToken && liveToken.length>0){
    return liveToken;
   }
...
const token_details={
  ...
  appId:liveToken?.appId || null,
    ...
    const teama={
      ...
      start_time:bet_list.matchId.start_time,
    ...
    if (outcome === "A") {
      const data = {
        winnerTeam:bet_list.winnerTeam || null,
        status:outcome,
        userId:comptId,
        team1: [
...
const generateToken = async(req, res) => {
  try {
          ...
          if (!coordinatorId || !matchId) {
            return res.status(400).json({
              result: "false",
              message: "Required fields are coordinatorId, matchId",
            });
          };
    ...
    const channelName =generateRandomString() ;
    //const uid =generate_udi() ;
    //const channelName = req.body.channelName || "testChannel"; // Accept channelName from request
    const uid = req.body.uid || 0; // Default UID to 0 for audience
    const currentTimestamp = Math.floor(Date.now() / 1000);
    const privilegeExpireTimestamp = currentTimestamp + (10 * 365 * 24 * 60 * 60); // 1000 hour token
  ...
  const findToken=await Live.findOne({coordinatorId,matchId});
  if(!findToken || findToken.length===0){

    const insertData = new Live({
      appId,
      token,
      channelName,
      uid,
    ...
    await Live.findOneAndUpdate({coordinatorId,matchId},{token,channelName},{new:true})
  }
    ...
    res.status(500).json({ result: "false", message: "Failed to generate token" });
  }
};
...
const deleteliveToken = async(req, res) => {
  try {
          const { coordinatorId, matchId} = req.body;
          if (!coordinatorId || !matchId) {
            return res.status(400).json({
    ...
    await Live.findOneAndDelete({coordinatorId,matchId});
...
const userDashboard=async(req,res)=>{
  try{
    ...
    const [allMatch,winMatch,winPrize,offerAmount]=await Promise.all([
      Bets.find({userId}).countDocuments(),
      Bets.find({userId,winning_amount:{$ne:null}}).countDocuments(),
      Bets.find({userId}),  

    ])
    const winningPrize=winPrize.reduce((total,item)=>total + (item.winning_amount || 0),0);
    const data={
      allMatch: allMatch || 0,
      winMatch: winMatch || 0,
      winPrize:winningPrize ||0,
...
const  coordinatorWithdrawRequest=async(req,res)=>{
  try{
  const {coordinatorId,amount,date}=req.body;
  if(!coordinatorId || !amount){
    return res.status(400).json({result:"false",message:"Required field is coordinatorId,amount,date"});
  }
  const coordinator=await Coordinator.findOne({_id:coordinatorId});
  if(!coordinator){
    return res.status(400).json({result:"false",message:"Invalid coordinatorId"})
  }
  const walletAmount=coordinator.coordinatorWallet;
  if(!(walletAmount>=amount)){
 return res.status(400).json({result:"false",message:"Insufficient blance"})
  }
  const insertData=new Withdraw({coordinatorId,amount,date});
  const data=await insertData.save();
  res.status(200).json({result:"true",message:"Withdraw request successfully",data:data})
...
const  coordinatorWithdrawList=async(req,res)=>{
  ...
  const {coordinatorId}=req.body;
  if(!coordinatorId){
    return res.status(400).json({result:"false",message:"Required field is coordinatorId"});
  }
  
  const data= await Withdraw.find({coordinatorId}).sort({createdAt:-1});
  if(!data || data.length ===0){
    return res.status(400).json({result:"false",message:"Record not found"});
  }
  res.status(200).json({result:"true",message:"Withdraw request successfully",data:data})
